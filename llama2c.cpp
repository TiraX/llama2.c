...
void malloc_run_state(RunState* s, Config* p) {
    ...
    //s->xq = (QuantizedTensor){ .q = (float*)calloc(p->dim, sizeof(int8_t)), .s = (float*)calloc(p->dim, sizeof(float)) };
    //s->hq = (QuantizedTensor){ .q = (float*)calloc(p->hidden_dim, sizeof(int8_t)), .s = (float*)calloc(p->hidden_dim, sizeof(float)) };
    s->xq.q = (int8_t*)calloc(p->dim, sizeof(int8_t));
    s->xq.s = (float*)calloc(p->dim, sizeof(float));
    s->hq.q = (int8_t*)calloc(p->hidden_dim, sizeof(int8_t));
...
void quantize(QuantizedTensor* qx, float* x, int n) {
    ...
    for (int group = 0; group < num_groups; group++) {
        ...
        float wmax = 0.0;
        for (int i = 0; i < GS; i++) {
            float val = (float)fabs(x[group * GS + i]);
            if (val > wmax) {
                wmax = val;
            }
        }
...
void encode(Tokenizer* t, char* text, int8_t bos, int8_t eos, int* tokens, int* n_tokens) {
    ...
    *n_tokens = 0;
    ...
    if (text[0] != '\0') {
        ...
        temp_str[0] = ' ';
        temp_str[1] = 0;
        int dummy_prefix = str_lookup(temp_str, t->sorted_vocab, t->vocab_size);
...
void generate(Transformer* transformer, Tokenizer* tokenizer, Sampler* sampler, char* prompt, int steps) {
    ...
    // start the main loop
    clock_t start = 0;  // used to time our code, only initialized after first iteration
...
int main(int argc, char* argv[]) {
    ...
    char c_tokenizer_path[] = "tokenizer.bin";
    char* tokenizer_path = c_tokenizer_path;
    float temperature = 1.0f;   // 0.0 = greedy deterministic. 1.0 = original. don't set higher
    float topp = 0.9f;          // top-p in nucleus sampling. 1.0 = off. 0.9 works well, but slower
    int steps = 256;            // number of steps to run for
    char* prompt = NULL;        // prompt string
    unsigned long long rng_seed = 0; // seed rng with time by default
    char c_mode[] = "generate";    // generate|chat
    char* mode = c_mode;
